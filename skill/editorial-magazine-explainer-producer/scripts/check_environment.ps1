...
$checks = @(
    ...
    Get-ToolResult -Name 'GitHub CLI（私有仓库访问）' -Command 'gh' -VersionArgs @('--version') -Required $false
)
...
Write-Host "[$configMark] 本机工作区配置：$($configStatus.path)"
if (-not $configStatus.valid) {
    Write-Warning 'Skill 已安装，但环境或工作区配置尚不完整；缺项不会被自动安装。'
}
