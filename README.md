...
这是 `editorial-magazine-explainer-producer` 的私有发行仓库。它把已经验收的二维杂志插画视频流程固化为可迁移的 Codex Skill，包括：真实口播母钟、SRT、全量 Gate 2 审核、独立语义组件、解释文字、Google Flow 分阶段交接、青年明亮音、背景音乐、按资产类别放大与中部集中、字幕层级/对比度/主视觉避让、返工锁定、全片 QA、桌面哈希交付和发布闭环。

当前版本 `1.1.0` 新增两组机器硬门槛：Flow、本地组件、整幅插画必须分别执行缩放策略；字幕必须同时通过舒适字号、中心方向位移、背景对比、风格一致性、单行边界和主视觉非遮挡审计。缺少审计记录或违规计数不为零时，校验器不会允许 Gate 3 通过。
